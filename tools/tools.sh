...
if docker exec "$SERVER_CONTAINER_NAME" bash -c "[ -d \"$PROJECT_DIR\" ]"; then
    ...
    # Same restyling as cert1.php, plus dropping a call to
    # show_share_buttons() -- removed from cert.inc when cert1.php was
    # restyled, but upstream's cert_team.php still called it, fatally
    # erroring on every load for any user whose account has a team.
    docker cp ./html/user/cert_team.php "$SERVER_CONTAINER_NAME":"$PROJECT_DIR/html/user/cert_team.php"
    ...
    docker cp ./html/user/universe.php "$SERVER_CONTAINER_NAME":"$PROJECT_DIR/html/user/universe.php"
    ...
    # BIMI (Brand Indicators for Message Identification) logo -- referenced
    # by the default._bimi.camicia.dev DNS TXT record (v=BIMI1; l=https://
    # camicia.dev/camicia/img/bimi.svg), not by any page here. Same artwork
    # as favicon.svg, but restricted to the SVG Tiny 1.2 "Portable/Secure"
    # profile BIMI requires: no scripts, no external references, no raster
    # images, and the diamond pip drawn as a real <path> instead of a
    # Unicode ♦ glyph rendered via a system font (font availability can't
    # be relied on across whatever renders this for BIMI validation).
    # Requires DMARC at enforcement (p=quarantine/p=reject) to actually be
    # honored by mail clients -- see RUNBOOK.md if that's not already true
    # when reading this later.
    docker cp ./html/user/img/bimi.svg "$SERVER_CONTAINER_NAME":"$PROJECT_DIR/html/user/img/bimi.svg"
