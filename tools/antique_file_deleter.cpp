...
int delete_antiques_from_dir(char*dirpath, time_t mtime, uid_t uid) {
    ...
    // scan directory
    //
    // Camicia fix: errno must be reset to 0 immediately before *each*
    // readdir() call, not just once before the loop starts. The original
    // upstream code only reset it once; every iteration's check_stop_daemons()
    // call below probes for a stop_daemons file via stat(), which normally
    // (and correctly) gets ENOENT when no stop has been requested -- but that
    // leaves errno set to ENOENT as a side effect. On the final iteration,
    // once readdir() genuinely reaches end-of-directory (a normal, non-error
    // NULL return that does not itself touch errno), the post-loop "if
    // (errno)" check below sees that stale ENOENT from check_stop_daemons()
    // and misreports a perfectly successful, complete directory scan as
    // "Couldn't read dir". Confirmed via strace: readdir() itself never
    // fails; check_stop_daemons()'s own stat("./config.xml", ...)/
    // stat("../stop_daemons", ...) probes are what leave errno == ENOENT.
    // No upstream GitHub issue found for this as of 2026-08-20.
    while (1) {
        errno = 0;
        entry = readdir(dir);
        if (!entry) break;
