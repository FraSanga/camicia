...
have been restored by ".$g_logged_in_user->name."\n";
            ...
            foreach ($emails as $email) {
                // Camicia fix: $admin was never initialized -- stock
                // BOINC relied on PHP's old auto-vivify-on-property-
                // assignment behavior to turn "$admin->email_addr = ..."
                // into a fresh stdClass. PHP 8 removed that and throws
                // "Attempt to assign property \"email_addr\" on null"
                // instead, fatally, on every single suspend/unsuspend.
                // ->name is set too (not left null) so send_email()'s
                // PHPMailer::addAddress() call doesn't hand a null into
                // a string parameter further down -- same class of bug
                // as the strlen(null) fix in login_form.php.
                $admin = new stdClass();
