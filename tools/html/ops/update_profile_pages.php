...
function build_alpha_pages() {
    ...
    $numIds = 0;
    $members = array();
    // Camicia fix: only ever assigned inside the foreach below, so with
    // zero named profiles (true for every project's first several runs,
    // including ours until real profiles existed) the loop body never
    // executes and this stayed completely undefined -- a real
    // "Undefined variable $letters_used" warning on line 328's call,
    // confirmed live on every single run in update_profile_pages.out.
    // Harmless in effect (build_alpha_summary_page() handles an empty
    // array identically to how PHP auto-null'd the undefined variable),
    // but a real warning nonetheless. Found live during profiles testing.
    $letters_used = array();
