...
";
    ...
    $body = email_greeting($founder)."
Team member ".$user->name." has asked that you
...
The founder of ".$team->name." has declined your request
...
case "initiate_transfer":
    ...
    if (new_transfer_request_ok($team, $now)) {
        ...
        // Camicia fix: this used to unconditionally claim the founder was
        // notified "by email and private message" even when $success was
        // false. The PM (the channel that actually matters here) always
        // sends regardless, so this stays non-blocking by design -- just
        // honest about the email leg specifically now.
        if ($success) {
...
case "decline":
    ...
    if ($team->ping_user) {
        ...
        // Camicia fix: this never checked send_founder_transfer_decline_email()'s
        // return value, so the page always claimed the decline was
        // communicated even if the notice email failed -- confirmed live
        // on staging 2026-09-05 (same pattern as the other false-success
        // bugs in this codebase). The decline itself already committed to
        // the DB above, so it stays non-blocking, just honest now.
