...
function add_admin($team) {
    ...
    $now = time();
    // Camicia fix: same class of bug as team_forum.php's create_forum() --
    // upstream's INSERT never sets `rights`, and team_admin.rights is NOT
    // NULL with no default. Silently fine under a lenient SQL mode
    // (implicitly inserts 0), an uncaught mysqli_sql_exception ("Field
    // 'rights' doesn't have a default value") under STRICT_TRANS_TABLES,
    // this project's DB's actual mode. Confirmed present verbatim at the
    // exact BOINC commit this project's image builds from, and confirmed
    // `rights` isn't actually read anywhere in this codebase (or upstream's
    // own team_admins.php/team.inc), so 0 matches what a lenient mode would
    // have done anyway and has no behavioral effect either way.
