...
function show_platforms() {
    // Camicia fix: get_cached_data() returns null on a cold/expired
    // cache, and passing null to unserialize() is deprecated as of PHP
    // 8.1 -- worth fixing here specifically since this file is the
    // scheduler's own get_project_config RPC response: an unguarded
    // deprecation notice would get injected straight into the raw XML
    // every BOINC client parses on attach, not just shown on a web page
    // (see tools/server_status.php for the same fix, found first).
