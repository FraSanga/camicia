...
| 0003 | [`create_camicia_completed_ranges`](0003_create_camicia_completed_ranges.sh) | Creates `camicia_completed_ranges` table to maintain a permanent structured ledger of completed permutation blocks with volunteer attribution (`user_id`, `verifier_user_id`) | Eliminates flat-file scanning for range lookups, provides instant sub-millisecond gap/continuity verification, and enables lifetime volunteer portfolio statistics independent of BOINC's 7-day `db_purge` | **Applied 2026-09-22** -- table created on production, completed ranges backfilled from results.txt |
| 0004 | [`create_camicia_histogram_totals`](0004_create_camicia_histogram_totals.sh) | Creates `camicia_histogram_totals` table (arbitrary-precision DECIMAL columns) and adds `histogram_applied` flag to `camicia_completed_ranges` | Stores macro-distribution counts across all 64 game-length buckets and aggregate sums ($\sum C, \sum T, \sum C^2, P_1$) with transactional crash-safety and zero double-counting | **Pending deployment** |
